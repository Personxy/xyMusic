<template>
  <div class="musiclyric" v-show="songdetailflag">
    <!-- 唱片和拨片 -->
    <div class="discimg">
      <!-- 拨片 -->
      <div :class="{ disctriger: playstatus, nodisctriger: !playstatus }">
        <img src="../../../assets/images/player_bar.png" alt="" />
      </div>
      <!-- 唱片 -->
      <div :class="{ discroate: playstatus, nodiscroate: !playstatus }">
        <img src="../../../assets/images/disc.png" alt="" />
        <el-image
          :src="songDetails.al.picUrl"
          alt=""
          class="detailcover"
        ></el-image>
      </div>
    ...
    <div class="lyricbox">
      ...
      <div class="lyrictitle">
        <div
          :class="{ titleactive: titlewidth > 650, titlebox: true }"
          ref="lyrictitleinfo"
        >
          {{ songDetails.name }}
        </div>
      ...
      <div class="songauthorandalubm">
        <span @click="tosingerpage(songDetails.ar[0].id)" class="detailsinger"
          >{{ songDetails.ar[0].name }}-</span
        >
        <span @click="toalbumpage(songDetails.al.id)" class="detailalbum">{{
          songDetails.al.name
        }}</span>
      </div>
      <!-- 歌词展示 -->
      <div class="lryicul" ref="lyricScroll">
        <ul ref="lyric" class="lyric" v-if="lrcObject.length > 1">
          <li
        ...
        <div class="div" v-else>纯音乐，请欣赏</div>
      </div>
    </div>
  </div>
</template>
