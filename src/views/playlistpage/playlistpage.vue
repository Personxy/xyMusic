<template>
  <div class="playlistpage">
    ...
    <!-- 歌单导航 -->
    <listnav :songs="songs" v-loading="loading" :likeplaylist="likeplaylist" :playlist="playlist"  @togetsearchdata="getsearchdata"/>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async getallplaylist() {
      ...
      this.firstsonglist=resdata.data.songs
      // 加载动画结束
      this.loading = false;
    },
    ...
    getsearchdata(data) {
      console.log(data);
      this.songs=this.firstsonglist.filter(item=>{
        return item.name.indexOf(data)>-1
      })
      if(data.trim()==''){
        this.songs=this.firstsonglist
      }
    },
  },
  created() {
