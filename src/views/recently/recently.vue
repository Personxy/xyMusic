<template>
  <div class="recomsongs">
    ...
    <div class="recomtable">
      <el-table :data="recentsongs" stripe style="width: 100%" @row-dblclick="getmusic" @cell-mouse-enter="hovertitle(true)" @cell-mouse-leave="hovertitle(false)" @cell-click="tosingerpage">
...
export default {
  ...
  methods: {
    ...
    async playallmusic() {
      ...
      this.$store.dispatch('saveplaysonglist', this.recentsongs);

      // 获取当前列表第一首
      const res = await this.$http.get('/song/url', {
        params: {
          id: this.recentsongs[this.index].id,
          // cookie: this.cookie,
        },
      });
      ...
      if (!res.data.data[0].url) {
        ...
        const resdata = await this.$http.get('/song/detail', {
          params: {
            ids: this.recentsongs[this.index].id,
          },
        });
        // console.log(resdata);
        // 存入歌曲详情
